...
int main(int argc, char *argv[])
{
    ...
    //Инициализация ключа
    //--------------------------------------------------------------------
    BYTE* rawKey = (BYTE*)malloc(64 * sizeof(BYTE));
    for(int i = 0; i < 64; i++)
    {
        rawKey[i] = 0;
    }
    ...
    //if(!CryptCreateHash(hProv, CALG_GR3411_2012_256_HMAC_FIXEDKEY, 0, 0, &hHash))
    if(!CryptCreateHash(hProv, CALG_GR3411_2012_256_HMAC_FIXEDKEY, 0, 0, &hHash))
    {
    ...
    CryptSetHashParam(hHash, HP_HMAC_FIXEDKEY, 0, 0);
